...
public interface JpaPaymentOutboxRepository extends JpaRepository<PaymentOutboxEntity, UUID> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<PaymentOutboxEntity> findAndLockPendingMessages(@Param("batchSize") int batchSize);

    /**
     * Update the status of an outbox message.
     */
    @Modifying
    @Query("UPDATE PaymentOutboxEntity o SET o.status = :status, o.processedAt = :processedAt WHERE o.id = :id")
     ...
     */
    @Modifying
    @Query("UPDATE PaymentOutboxEntity o SET o.retryCount = o.retryCount + 1 WHERE o.id = :id")
     ...
     */
    @Modifying
    @Query("DELETE FROM PaymentOutboxEntity o WHERE o.status = 'PROCESSED' AND o.processedAt < :cutoffDate")
